...
const Player = () => {
  const { enrolledCourses, calculateChapterTime, backendUrl, getToken, userData, fetchUserEnrolledCourse } = useContext(AppContext)
                  ...
                  <ul className='list-disc md:pl-10 pl-4 pr-4 py-2 text-gray-600 border border-gray-300'>
                    {chapter.chapterContent.map((lecture, i) => (
                      <li key={i} className='flex item-start gap-2 py-1'>
                        <img src={progressData.lecturesCompleted && progressData.lecturesCompleted.includes(lecture.lectureId) ? assets.blue_tick_icon : assets.play_icon} alt="play-icon" className='w-4 h-4 mt-1' />
              ...
              <div >
                ...
                <div className='flex justify-between items-center mt-1'>
                  ...
                  <button onClick={() => markLectureAsCompleted(playerData.lectureId)} className='text-blue-600'>
                    {progressData.lecturesCompleted && progressData.lecturesCompleted.includes(playerData.lectureId)
                      ? 'Completed'
                      : 'Mark As Complete'}
                  </button>
                </div>
