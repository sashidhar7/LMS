...
const CourseDetails = () => {
  ...
  //   When you access openSections[index] and the index doesn't exist in the object, it will return undefined.

  // In JavaScript, undefined is falsy, meaning that it will behave like false when evaluated in a conditional check. 
  ...
  return courseData ? (
    ...
    justify-between md:px-36 px-8 md:pt-20 pt-20 text-left'>
        {/* Left Column */}
        <div className='absolute top-0 left-0 w-full h-[500px] bg-gradient-to-b from-cyan-100/70'>

        </div>
        <div className='max-w-xl z-10 text-gray-500'>
          <h1 className='md:text-[30px] text-[26px] font-semibold text-gray-800'>{courseData.courseTitle}</h1>
          <p className='pt-4  text-base text-sm'
          ...
          <p className='text-sm'>Course By <span className='text-blue-600 underline'>Sashidhar</span></p>
          <div className='pt-8 text-gray-800'>
            <h2 className='text-xl font-semibold'>Course Structure</h2>
            <div className='pt-5'>
              {courseData.courseContent.map((chapter, index) => (
                <div key={index} className='border border-gray-300 bg-white mb-2 rounded'>
                  <div className='flex items-center justify-between px-4 py-3' onClick={() => toggleSection(index)}>
                  ...
                  <div className={`overflow-hidden transition-all duration-300 ${openSections[index] ? 'max-h-96' : 'max-h-0'} `}>
                    <ul className='list-disc md:pl-10 pl-4 pr-4 py-2 text-gray-600 border border-gray-300'>
                      {chapter.chapterContent.map((lecture, index) => (
                        <li key={index} className='flex item-start gap-2 py-1'>
        ...
        {/* Right Column */}
        <div className='max-w-[424px] z-10 shadow-md border border-none rounded md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]'>
          {
          ...
          <div className='p-5'>
            ...
            <div className='flex gap-3 items-center pt-2'>
              ...
              <p className='md:text-lg text-gray-5000'>{courseData.discount}% off</p>
